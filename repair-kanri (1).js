...
function updateShoot(g){
  ...
  if(n){ b.disabled=false; b.textContent='次：'+n+' を撮る／アルバムから選ぶ'; b.onclick=function(){shoot(g,n,true);}; }
  else { b.disabled=true; b.textContent=(g==='B'?'ビフォー':'アフター')+'は全部そろった'; b.onclick=null; }
}
function shoot(g,s,multi){
  var fi=$('file');
...
function onPick(input){
  ...
  var p=pending;
  var list=p.g==='B'?photosB:photosA;
  var slots=curSlotsB();

  /* 入れる枠を決める。1枚ならタップした枠、複数なら押した枠から空いている順 */
  var targets=[];
  if(files.length===1){
    targets=[p.s];
  }else{
    var start=slots.indexOf(p.s); if(start<0) start=0;
    for(var i=start;i<slots.length && targets.length<files.length;i++){
      if(!findP(list,slots[i])||slots[i]===p.s) targets.push(slots[i]);
    }
  }
  files=files.slice(0,targets.length);

  var done=0, total=files.length;
  $('load').textContent='写真を送信中 0/'+total; $('load').classList.add('on');
  ...
  function next(k){
    ...
    var slot=targets[k];
    shrink(files[k], function(data, mime){
      call('uploadPhoto',[{mime:mime,data:data,itemId:(cur?cur.ID:''),slot:slot}],function(id){
        var ex=findP(list,slot);
        if(ex) ex.id=id; else list.push({s:slot,id:id});
        done++;
        $('load').textContent='写真を送信中 '+done+'/'+total;
        drawSlots();
        next(k+1);
      },function(e){
        $('load').classList.remove('on'); drawSlots(); updateWarn();
        alert(slot+' の写真を送れなかった：'+e.message);
      });
    });
  }
  next(0);
...
/* 読み込んだ時点でカメラ固定を外す */
(function(){ var fi=document.getElementById('file'); if(fi) fi.removeAttribute('capture'); })();
