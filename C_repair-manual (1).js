...
function mount(tries){
 ...
 /* ツール欄から準備中の2枚を消す（利益シミュレーター／修理パーツ・工具リスト） */
 var KILL=['利益シミュレーター','修理パーツ'];
 [].slice.call(host.querySelectorAll('.tool')).forEach(function(el){
  var tx=el.textContent||'';
  for(var i=0;i<KILL.length;i++){
   if(tx.indexOf(KILL[i])>=0){el.parentNode.removeChild(el);return}
  }
 });

 /* タブを追加（動画の前＝ツール／マニュアル／動画の並び） */
 var tab=document.createElement('button');
 tab.className='subtab rp-tab';
 tab.setAttribute('data-pane','manual');
 tab.textContent='📗 マニュアル';
 var movieTab=row.querySelector('[data-pane="movie"]');
 if(movieTab)row.insertBefore(tab,movieTab);else row.appendChild(tab);
 ...
 var moviePane=document.getElementById('rp-movie');
 if(moviePane&&moviePane.parentNode===host.parentNode){
  host.parentNode.insertBefore(PANE,moviePane);
 }else{
